...
dt = 0.01;
steps = (t_vals/dt);
f_vector = repelem(f_vals,round(steps));
time_axis = linspace(0,length(f_vector)*dt,length(f_vector));
tf = time_axis;
O2_f = f_vector;
tend = tf(end);
starttest = sum(t_vals(1:end-1));
starttest_ind = round(starttest/dt);
s = round(tend/dt);
t = zeros(1,2);
...
for sq = 1:round(s)
    ...
    if mod(sq,10) == 0 % Store dissolved oxygen value every 10 iterations
...
diff_c = diff(c_o)/(t_o(2)-t_o(1)); % Derivative of dissolved oxygen profile
...
start_ind = find(abs(t_o - starttest) < 0.1); % Index associated with end of run time for current segment
start_ind = start_ind(1);
c_after = c_o(start_ind:end); % Dissolved oxygen profile beyond current segment run time

if toggle == 1 % For decreasing oxygen segment
    if no_inflection > 1
        f(1) = c_o(indices(end))*1e9 - target;
    else
        f(1) = min(c_after)*1e9 - target;
    end
    f(2) = inflection;
    if no_inflection > 1
        f(3) = target_t - (t_o(indices(end)) - t_o(indices(end-1)));
    else
        f(3) = 100; % Placeholder value to disregard possible solution set
    end
elseif toggle == 0 % For increasing oxygen segment
    if no_inflection > 1
        f(1) = target - c_o(indices(end))*1e9;
    else
        f(1) = target - max(c_o)*1e9;
    end
    f(2) = inflection;
    if no_inflection > 1
        f(3) = target_t - (t_o(indices(end)) - t_o(indices(end-1)));
